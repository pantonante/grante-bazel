...
int main() {
    ...
    for (unsigned int vi = 0; vi < var_names.size(); ++vi) {
        ...
        Grante::FactorType *factortype = new Grante::FactorType("pi_" + std::to_string(vi), {2}, w);
    ...
    for (unsigned int ti = 0; ti < test_scopes.size(); ++ti) {
        ...
        Grante::FactorType *factortype = new Grante::FactorType("t_" + std::to_string(ti), card, w);
        model.AddFactorType(factortype);
    }
    ...
    Grante::ContrastiveDivergenceTraining cdt(&model, 100, 10, 0.01);
